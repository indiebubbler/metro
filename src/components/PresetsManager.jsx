...
class PresetsManager extends Component {
  ...
  constructor(props) {
    ...
    this.state.showDelete = this.state.userPresets
      ? Array(this.state.userPresets.length).fill(false)
      : [];
    this.state.showEdit = this.state.userPresets
      ? Array(this.state.userPresets.length).fill(false)
      : [];
  }
  ...
  validatePreset(preset) {
    preset.byBarInterval =
      preset.byBarInterval || preset.interval || InitPreset.byBarInterval;
    preset.byTimeInterval =
      preset.byTimeInterval || preset.interval || InitPreset.byTimeInterval;
    ...
    if (!preset.timeSignature) {
      ...
      var track = new Array(Config.TRACKS_NUMBER)
        .fill(0)
        .map((x) => new Array(timeSignature).fill(0));
  ...
  onSavePreset(title, o) {
    ...
    if (preset.presetId === undefined) {
      ...
      if (this.state.userPresets.length > 0) {
        // find max and add 1
        presetId = Math.max.apply(
          Math,
          this.state.userPresets.map(function (p) {
            return p.presetId;
          })
        );
        presetId++;
    ...
    let idx = this.state.userPresets.findIndex(
      (o) => o.presetId === preset.presetId
    );
  ...
  onPresetDelete(preset) {
    ...
    if (idx < 0) {
      throw new Error(
        "Selected preset " + preset.title + " has not been found in the store"
      );
    }
  ...
  onPresetEdit(e, idx) {
    ...
    if (idx !== undefined) {
      this.refs.presetEditor.edit(
        p,
        this.state.userPresets[idx].presetId,
        this.state.userPresets[idx].title
      );
    } else {
  ...
  onExport() {
    ...
    const file = new Blob([JSON.stringify(this.state.userPresets)], {
  ...
  render() {
    ...
    return (
      <div className="PresetsManager">
        <SimplePanel title={Tr("Presets")}>
          <Container className="presets-container">
            {PresetsLib.map((item, idx) => {
              if (item.isHeader) {
                return (
                  <Row className={"presetHeader"} key={"header_" + idx}>
                    {Tr(item.title)}:
                  </Row>
                );
              } else {
            ...
            <Row style={{ justifyContent: "center" }}>
              <EditPresetModal
                ref="presetEditor"
                onDeleteBtn={(preset) => this.onPresetDelete(preset)}
                onSaveBtn={(e, idx) => this.onPresetEdit(e, idx)}
                onSave={(title, preset) => this.onSavePreset(title, preset)}
              />
              <ImportDialog
                ref="importDialog"
                onImportBtn={() => this.onImportBtn()}
                onJsonReady={(presets) => this.onPresetsImport(presets)}
              />
              <Button
